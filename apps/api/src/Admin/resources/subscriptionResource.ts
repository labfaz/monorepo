...
import {
  sendAprovedEmail,
  sendNotAprovedEmail,
} from "Mailer/courseSubscriptionResponse";
        ...
        updateStatus: {
          ...
          handler: async (request, _response, context) => {
            ...
            const [course, user] = await Promise.all([
              courseResource.findOne(courseId),
              userResource.findOne(studentId),
            ]);
            ...
            const showName =
              user.params["artist.show_name"] === ShowName.ARTISTIC
                ? "artistic_name"
                : user.params["artist.show_name"] === ShowName.SOCIAL
                ? "social_name"
                : "name";
            ...
            try {
              if (status === "accepted" || status?.status === "accepted") {
                const courseDates = Object.values(
                  (course.selectParams("class_dates") ?? {}) as {
                    [s: string]: any;
                  }
                );
                const courseLink = course.params.link;
                await sendAprovedEmail(
                  userEmail,
                  userName,
                  courseName,
                  courseLink,
        ...
        getOpenSubscriptions: {
          ...
          handler: async (request, response) => {
            ...
            const requests = await requestRepo
              .getOpenRequests(courseId)
              .then((req) => {
                console.log("GET SUBSCRIPTION RESOURCE", req[0].student);
                ...
                req.map((r) => {
                  subscriptions.push({
                    ...
                    student: {
                      ...
                      whatsapp: r.student.artist.contact.whatsapp ?? undefined,
                      name: r.student.artist.displayName,
                    },
                  });
                });
